...
class Mtable extends React.Component<any> {
  ...
  showModal = (text: textData, record: any) => {
    console.log(text);
    console.log(record);
    store.dispatch({ type: "GET_COLUMN", data: {
      userName: text.name
    } });
  ...
  render() {
    const columns: any = [
      ...
      { title: "使用状态", dataIndex: "address", key: "2" },
      {
        ...
        (<div><span onClick={() => { this.showModal(text, record) }}><Icon style={{ fontSize: '24px', color: '#08c' }} type="edit" /></span>
        <span className='left5'><Icon style={{ fontSize: '24px', color: '#08c'}} type="delete" /></span></div>)
      }
    ];
